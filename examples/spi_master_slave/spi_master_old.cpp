...
const uint32_t spi_rx_dmac_channel_settings =
    (DMAC_CHCTRLA_BURSTLEN_SINGLE | DMAC_CHCTRLA_TRIGSRC(SERCOM1_DMAC_ID_RX) |
     DMAC_CHCTRLA_TRIGACT_BURST);

const uint16_t spi_rx_dmac_descriptor_settings =
    (DMAC_BTCTRL_BEATSIZE_WORD | DMAC_BTCTRL_DSTINC |
     DMAC_BTCTRL_BLOCKACT_BOTH | DMAC_BTCTRL_VALID);
...
const uint32_t spi_tx_dmac_channel_settings =
    (DMAC_CHCTRLA_BURSTLEN_SINGLE | DMAC_CHCTRLA_TRIGACT_BURST |
     DMAC_CHCTRLA_TRIGSRC(SERCOM1_DMAC_ID_TX));

const uint16_t spi_tx_dmac_descriptor_settings =
    (DMAC_BTCTRL_VALID | DMAC_BTCTRL_BLOCKACT_BOTH | DMAC_BTCTRL_BEATSIZE_WORD |
     DMAC_BTCTRL_SRCINC);

void spi_dma_master_init(void) {
  DMAC_init(&base_descriptor[0], &wb_descriptor[0]);

  DMAC_channel_init(SPI_RX_DMAC_CHANNEL, spi_rx_dmac_channel_settings, 0);

  DMAC_descriptor_init(spi_rx_dmac_descriptor_settings, SPI_RX_BUFFER_LEN,
                       (uint32_t)&SERCOM1->SPI.DATA.reg,
                       (uint32_t)spi_rx_buffer +
                           sizeof(uint32_t) * SPI_RX_BUFFER_LEN,
                       (uint32_t)&base_descriptor[SPI_RX_DMAC_CHANNEL],
                       &base_descriptor[SPI_RX_DMAC_CHANNEL]);

  DMAC_channel_intenset(SPI_RX_DMAC_CHANNEL, DMAC_0_IRQn, DMAC_CHINTENSET_TCMPL,
                        2);

  DMAC_channel_init(SPI_TX_DMAC_CHANNEL, spi_tx_dmac_channel_settings, 0);

  DMAC_descriptor_init(spi_tx_dmac_descriptor_settings, SPI_TX_BUFFER_LEN,
                       (uint32_t)spi_tx_buffer +
                           sizeof(uint32_t) * SPI_TX_BUFFER_LEN,
                       (uint32_t)&SERCOM1->SPI.DATA.reg,
                       (uint32_t)&base_descriptor[SPI_TX_DMAC_CHANNEL],
                       &base_descriptor[SPI_TX_DMAC_CHANNEL]);

  DMAC_channel_intenset(SPI_TX_DMAC_CHANNEL, DMAC_1_IRQn, DMAC_CHINTENSET_TCMPL,
                        2);
 ...
 *
 */
ml_pin_settings pad1 = {PORT_GRP_A,       1,        PF_D, PP_ODD,
                        OUTPUT_PULL_DOWN, DRIVE_OFF};
ml_pin_settings pad2 = {PORT_GRP_B,       22,       PF_B, PP_EVEN,
                        OUTPUT_PULL_DOWN, DRIVE_OFF};
ml_pin_settings pad3 = {PORT_GRP_B,     23,       PF_C, PP_ODD,
                        INPUT_STANDARD, DRIVE_OFF};
ml_pin_settings pad0 = {PORT_GRP_A,       0,        PF_D, PP_EVEN,
                        OUTPUT_PULL_DOWN, DRIVE_OFF};
...
void ss_timer_init(void) {
  ...
  TC0->COUNT16.CTRLA.reg = (TC_CTRLA_PRESCALER_DIV1 | TC_CTRLA_MODE_COUNT16 |
  ...
  // PA04 --> A4 (IB)
  ml_pin_settings ss_timer_pin = {PORT_GRP_A,       4,       PF_E, PP_EVEN,
                                  OUTPUT_PULL_DOWN, DRIVE_ON};
...
void SERCOM1_1_Handler(void) { logical_set(&pad2); }
