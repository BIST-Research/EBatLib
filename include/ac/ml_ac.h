...
#define ML_AC_SET_CONT_MEASURE(channel)                                        \
  (AC->COMPCTRL[channel].reg &= ~AC_COMPCTRL_SINGLE)

#define ML_AC_SET_SINGLE_MEASURE(channel)                                      \
  (AC->COMPCTRL[channel].reg |= AC_COMPCTRL_SINGLE)
...
/**
 * @brief Synchronizes the AC peripheral
 */
void AC_sync(void);

/**
 * @brief Initializes the AC peripheral
 */
void AC_init(void);
