...
/**
 * @brief this struct defines the settings of a DMA channel
 */
...
#define ML_DMAC_CHANNEL_ENABLE(channel)                                        \
  (DMAC->Channel[channel].CHCTRLA.reg |= DMAC_CHCTRLA_ENABLE)
#define ML_DMAC_CHANNEL_DISABLE(channel)                                       \
  (DMAC->Channel[channel].CHCTRLA.reg &= ~DMAC_CHCTRLA_ENABLE)
#define ML_DMAC_CHANNEL_SWRST(channel)                                         \
  (DMAC->Channel[channel].CHCTRLA.reg |= DMAC_CHCTRLA_SWRST)

#define ML_DMAC_CHANNEL_RESUME(channel)                                        \
...
#define ML_DMAC_CHANNEL_INTSET_SUSP(channel)                                   \
  (DMAC->Channel[channel].CHINTENSET.bit.SUSP = 0x01)
#define ML_DMAC_CHANNEL_INTSET_TERR(channel)                                   \
...
#define ML_DMAC_CHANNEL_SUSP_INTFLAG(channel)                                  \
  (DMAC->Channel[channel].CHINTFLAG.bit.SUSP == 0x01)
#define ML_DMAC_CHANNEL_TERR_INTFLAG(channel)                                  \
  (DMAC->Channel[channel].CHINTFLAG.bit.TERR == 0x01)

#define ML_DMAC_CLEAR_DESCRIPTOR_VALID_BIT(settings)                           \
  (settings &= ~DMAC_BTCTRL_VALID)
#define ML_DMAC_SET_DESCRIPTOR_VALID_BIT(settings)                             \
  (settings |= DMAC_BTCTRL_VALID)

void DMAC_channel_intenset(const ml_dmac_chnum_t channel, const IRQn_Type IRQn,
                           const uint8_t intmsk, const uint32_t priority_level);
