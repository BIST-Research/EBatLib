...
void peripheral_port_init(const ml_pin_settings *set) {
  ...
  uint8_t pmux_mask =
      parity ? PORT_PMUX_PMUXE((uint8_t)func) : PORT_PMUX_PMUXO((uint8_t)func);

  PORT->Group[group].PINCFG[pin].reg |= PORT_PINCFG_PMUXEN;

  PORT->Group[group].PINCFG[pin].reg |=
      (CONFIG_PULLEN(config) ? PORT_PINCFG_PULLEN : 0x0);

  PORT->Group[group].PINCFG[pin].reg |=
      (CONFIG_INEN(config) ? PORT_PINCFG_INEN : 0x0);

  PORT->Group[group].PINCFG[pin].reg |= (drive ? PORT_PINCFG_DRVSTR : 0x0);

  PORT->Group[group].PMUX[pin >> 1].reg |= pmux_mask;

  PORT->Group[group].DIR.reg |=
      (CONFIG_DIRSET(config) ? PORT_DIRSET_DIRSET(pin) : 0x0);
...
boolean logical_read(const ml_pin_settings *set) {
  return ((PORT->Group[set->group].IN.reg & (1 << PORT_IN_IN(set->pin))) >>
          PORT_IN_IN(set->pin));
}
