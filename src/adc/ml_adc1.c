...
void ADC1_init(void) {
  ...
  ADC1->INPUTCTRL.reg = (ADC_INPUTCTRL_MUXPOS_AIN1 | ADC_INPUTCTRL_MUXNEG_GND);
  while (ADC1->SYNCBUSY.bit.INPUTCTRL) {
    /* Wait for sync */
  }

  ADC1->CTRLB.reg =
      (ADC_CTRLB_RESSEL_12BIT | ADC_CTRLB_WINMODE(0U) | ADC_CTRLB_FREERUN);
