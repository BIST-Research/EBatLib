...
void TCC0_DT_set(uint8_t dth, uint8_t dtl) {
  TCC0->WEXCTRL.reg |=
      (TCC_WEXCTRL_DTIEN0 | TCC_WEXCTRL_DTLS(dtl) | TCC_WEXCTRL_DTHS(dth));
}

// mode: 4, 5, 6
void TCC0_DITH_set(uint8_t mode, uint64_t cycles, uint64_t period,
                   uint64_t compare) {
...
void TCC0_init_alt(void) {
   ...
   * regs SYNCED on write: CTRLBCLR, CTRLBSET, STATUS, WAVE, COUNT, PER/PERBUF,
   * CCx/CCBUFx regs SYNCED on read: CTRLBCLR, CTRLBSET, COUNT, WAVE,
   * PER/PERBUF, CCx/CCBUFx
   */
  ...
  TCC0->CTRLA.reg = TCC_CTRLA_PRESCALER_DIV1 |
                    TCC_CTRLA_PRESCSYNC_PRESC; // try TC_CTRLA_PRESYNC_GCLK or
                                               // TC_CTRLA_PRESYNC_RESYNC
                                               //               TCC_CTRLA_DMAOS;
  //               TCC_CTRLA_RESOLUTION_DITH4 |                           //
  //               enable dithering every 16 PWM frames TCC_CTRLA_RUNSTDBY; //
  //               run TCC when MP in standby

  // TCC0->CTRLA.reg &= ~TCC_CTRLA_DMAOS;
  // dead time reg
  /*
  TCC0->WEXCTRL.reg = TCC_WEXCTRL_DTIEN0       |                         // DT
  insertion gen 0 enable TCC_WEXCTRL_DTHS(0x1)      |                       //
  DT high side value set TCC_WEXCTRL_DTLS(0x1)      |                       //
  DT low side value set
  //                  TCC_WEXCTRL_OTMX(0x0);                             //
  default output matrix config
  */
   ...
   *
   * counter val continuously compared to each CC channel. When match,
   * INTFLAG.MCx is set
   */
  /*
  TCC0->INTENSET.reg = TCC_INTENSET_OVF |                               //
  overflow fault TCC_INTENSET_TRG |                                  // retigger
                    TCC_INTENSET_CNT |                                  //
  counter TCC_INTENSET_ERR |                                  // error
                    TCC_INTENSET_UFS |                                  //
  non-recoverable update fault TCC_INTENSET_FAULTA | // recoverable fault A/B
                    TCC_INTENSET_FAULTB |
                    TCC_INTENSET_FAULT0 |                               //
  non-recoverable fault 0/1 TCC_INTENSET_FAULT1 | TCC_INTENSET_MC(0)  | // Match
  or capture channel x
  */
  ...
  // TCCO->INTFLAG.reg                                                  // for
  // reading interrupt status (pg 1866) TCC0->STATUS.reg // reading tcc status
  // (pg 1868) TCC0->COUNT.reg // reading counter value, set
  // CTRLBSET.CMD=READSYNC prior to read
  ...
  //              TCC_PER_DITH4_DITHER(1) |                             //
  //              dithering cycle number TCC_PER_DITH4_PER(1)    | // period
  //              value set (if dithering enabled)
  ...
  // sync required on read and write
  /*
  TCC0->PERBUF.reg = TCC_PERBUF_PERBUF(1);                              // value
  copied to PER on UPDATE condition
  //                 TCC_PERBUF_DITH4_DITHERBUF(1) |                    //
  dithering buffer update bits
  //                 TCC_PERBUF_DITH4_PERBUF(1)    |                    //
  period update if dithering enabled while(TCC00>SYNCBUSY.bit.PERBUF);
  */

  // capture compare reg
  // sync require on read and write

  // TCC0->CC[ML_TCC0_CH0].reg = TCC_CC_CC(50);                             //
  // CC value (18 bits)
  //               TCC_CC_DITH4_DITHER(1)                                //
  //               dithering cycle number TCC_CC_DITH4_CC(1) // CC value (if
  //               dithering enabled)
  ...
  // inverts ch3 wave, we want complimentary outs

  // Channel x CC buffer value regs: CCBUFx (force update w/ CTRLBSET.CMD=0x3)
  // capture compare buffer reg TCC0->CCBUF.reg, similar to PERBUF (pg 1882),
  // NEEDS to wait for SYNC on read and write*
}
...
void TCC1_init(void)
{
  ...
  // Thus, make the TCC period = the maximum amplitude of the wavetable which is
0xff
  ...
  // TCC1->CC[ML_TCC1_CH3].reg = TCC_CC_CC((unsigned)(ML_TCC1_CH3_INITIAL_PER /
2));
