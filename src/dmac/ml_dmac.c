...
void DMAC_channel_intenset(const ml_dmac_chnum_t channel, const IRQn_Type IRQn,
                           const uint8_t intmsk,
...
void DMAC_init(DmacDescriptor *base_descriptor,
               volatile DmacDescriptor *writeback_descriptor) {
  ...
  DMAC->CTRL.reg |=
      DMAC_CTRL_LVLEN0 | DMAC_CTRL_LVLEN1 | DMAC_CTRL_LVLEN2 | DMAC_CTRL_LVLEN3;
}
 ...
 * into different channels, i.e. one channel for moving data from the ADC to
 * memory and another for moving data from memory to a TCC instance
 *
 */
void DMAC_channel_init(const ml_dmac_chnum_t channel, const uint32_t settings,
                       const ml_dmac_chprilvl_t priority_level) {
  ...
  // If the DMAC has two or more channels which need data moved, the priority
  // level of the channel tells the DMAC which to serve first
  DMAC->Channel[channel].CHPRILVL.reg = priority_level;
}

/*
 *
 * Initializes descriptors to be used by the direct memory access controller
 * (DMAC)
 *
 ...
 *
 */
void DMAC_descriptor_init(const uint16_t btsettings, const uint16_t btcnt,
                          const uint32_t srcaddr, const uint32_t dstaddr,
                          const uint32_t descaddr, DmacDescriptor *cpy) {
  ...
  // This is typically the length of the buffer we want to fill, or take data
  // from
  descriptor.BTCNT.reg = btcnt;

  // Location in memory of data to send. SRCADDR accepts last memory location of
  // data
  ...
  // What descriptor to point to when the DMAC is finished serving the current
  // descriptor. Typically we want to just point back to the same descriptor
...
void DMAC_descriptor_cpyto(DmacDescriptor *descriptor_target,
