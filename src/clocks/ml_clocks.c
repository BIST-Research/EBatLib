...
void MCLK_init(void) {
  ...
  // set peripheral bus access, so we can access their registers in setup and
  // main loop
...
void DPLL1_init(void) {
  ...
  OSCCTRL->Dpll[1].DPLLRATIO.reg =
      OSCCTRL_DPLLRATIO_LDR(DPLL1_INTEGER_MULTIPLICATION_VALUE);

  OSCCTRL->Dpll[1].DPLLCTRLA.bit.ENABLE = 0x1;
}

/*
 * Initializes the generic clock (GCLK7) to be used by peripherals (TCC, ADC,
 * AC, etc)
 */
void GCLK_init(void) {

  // GCLK divider, GCLK7_FREQ = ML_MCLK_UNDIV/(ML_MCLK_CPUDIV *
  // ML_GCLK_GENCTRL_DIV) 50/50 duty Source multiplexer selects DPLL1 Output
  // enable, Generator enable
  GCLK->GENCTRL[ML_GCLK_CH].reg = GCLK_GENCTRL_DIV(1) | GCLK_GENCTRL_IDC |
                                  GCLK_GENCTRL_SRC_DPLL0 | GCLK_GENCTRL_GENEN;
