...
void spi_dmac_tx_init(ml_dmac_s *dmac_s, Sercom *coms_inst,
  ...
  DMAC_channel_init(chnum, dmac_s->chan_settings, dmac_s->chan_prilvl);

  uint32_t btsize = DMAC_extract_btsize(dmac_s->descriptor_settings);

  DMAC_descriptor_init(dmac_s->descriptor_settings, dmac_s->ex_len,
                       (uint32_t)dmac_s->ex_ptr + (btsize * dmac_s->ex_len),
                       (uint32_t)&coms_inst->SPI.DATA.reg, (uint32_t)cpy, cpy);

  DMAC_channel_intenset(chnum, dmac_s->irqn, dmac_s->intmsk,
                        dmac_s->irqn_prilvl);
}
